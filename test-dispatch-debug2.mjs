...
(async () => {
  ...
  await page.evaluate(() => {
      setTimeout(() => {
          ...
          const state = Alpine.$data(basicRoot);
          
          // Let's manually run the EXACT code that Native Filament runs
          const editor = state.getEditor();
          let Y = editor.chain();
          
          const W = [ { name: 'setYoutubeVideo', arguments: [{ src: 'https://youtube.com/watch?v=dQw4w9WgXcQ' }] } ];
          
          try {
              W.forEach(F => {
                  console.log("F.name:", F.name);
                  console.log("Y[F.name] type:", typeof Y[F.name]);
                  Y = Y[F.name](...(F.arguments ?? []));
                  console.log("Returned Y:", Y ? "exists" : "undefined");
              });
              Y.run();
              console.log("Manual run succeeded!");
          } catch(e) {
              console.log("Manual run failed:", e.message);
          }

          console.log("Dispatching real event now...");
